...
const TesoreriaFiltro = () => {
    ...
    const cargaraniosacademicos = async () =>{
        try {
              ...
              setaniosacademicos(res.data); 
            } catch (error) {
              console.error("Error cargando anios:", error);
            } finally {
              setLoading(false);
            }
    }
  ...
  <div className="p-4 bg-white border-b flex items-center justify-center gap-4 text-sm font-medium text-gray-600">
          ...
          <div className="flex items-center gap-2">
            <span>Año</span>
            <select className="border rounded-full px-4 py-1 bg-gray-100">
                {aniosacademicos.length > 0 ? (
                    aniosacademicos.map((anio) => (
                        <option key={anio.id_periodo} value={anio.id_periodo}>{anio.anio}</option>
                    ))
                ) : (
                    <option></option>
                )}
